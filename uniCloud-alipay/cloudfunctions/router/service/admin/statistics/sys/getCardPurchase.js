module.exports = {
	/**
	 * 查询卡密购买统计
	 * @url admin/statistics/sys/getCardPurchase 前端调用的url参数地址
	 * data 请求参数 说明
	...
	main: async (event) => {
		...
		// 业务逻辑开始-----------------------------------------------------------
		let { user_id, product_id, start_time, end_time, formData } = data;
		...
		if (product_id) {
			whereJson.product_id = product_id;
		}
		...
		if (res.rows && res.rows.length > 0) {
			res.rows = res.rows.map(item => {
				...
				// 格式化产品类型
				if (item.card_type) {
					item.product_type_text = item.card_type === 'software' ? '软件' : '插件';
				} else {
					item.product_type_text = '未知';
				}
				
				// 格式化有效天数
				if (item.limit_days === -1) {
					item.limit_days_str = '永久有效';
				} else {
					item.limit_days_str = `${item.limit_days}天`;
				}
